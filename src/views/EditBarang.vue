<template>
    <div class="min-h-screen bg-slate-50 flex flex-col items-center p-10 font-sans">
        <div class="w-full max-w-2xl mb-8 border-b-2 border-indigo-100 pb-5 flex justify-between items-center">
            <div>
                <h1 class="text-blue-900 text-2xl font-extrabold tracking-tight uppercase">Edit Barang</h1>
            </div>
        ...
        <form @submit.prevent="edit"
            class="bg-white p-8 rounded-xl shadow-sm border border-slate-200 w-full max-w-2xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex justify-between text-[10px] uppercase font-bold tracking-tighter">
                        <span :class="form.nama.length >= 5 ? 'text-green-500' : 'text-slate-400'">Min. 5
                            Karakter</span>
            ...
            <div class="flex flex-col gap-2 mb-8">
                ...
                <textarea v-model="form.deskripsi" rows="4"
                    placeholder="Jelaskan detail spesifikasi atau kondisi barang..."
            ...
            <div v-if="form.nama" class="mb-8 p-4 bg-amber-50 border-l-4 border-amber-500 rounded">
                <h4 class="text-xs font-black text-amber-800 uppercase mb-2">Pratinjau Perubahan:</h4>
                <p class="text-slate-800 font-bold text-lg leading-tight">{{ form.nama }}</p>
                <p class="text-slate-600 text-xs mt-1 italic line-clamp-1">{{ form.kategori }}</p>
            </div>
            <button type="submit" :disabled="!isFormValid || isLoading"
                ...
                :class="isFormValid ? 'bg-orange-500 hover:bg-orange-600 shadow-orange-200 cursor-pointer' : 'bg-slate-300 cursor-not-allowed'">
                <span v-if="!isLoading">Simpan Perubahan</span>
                <span v-else class="animate-spin text-xl text-white">⏳</span>
            </button>
        </form>
    </div>
</template>
...
const isFormValid = computed(() => {
    return (
        form.nama.trim().length >= 5 &&
        form.kategori !== "" &&
        form.deskripsi.trim().length >= 15
    );
});

onMounted(async () => {
    const idBarang = route.params.id;
    await store.ambilBarangById(idBarang);

    if (store.barangTerpilih) {
        form.nama = store.barangTerpilih.nama;
        form.deskripsi = store.barangTerpilih.deskripsi;
        form.kategori = store.barangTerpilih.kategori;
    } else {
